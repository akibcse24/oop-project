...
void Admin::addBanker(string n) {
    ...
    while(getline(f,line))
        if(skip) { size_t t=line.find('\t')+1; string u=line.substr(t,line.find('\t',t)-t); if(u.find("-")!=string::npos) id=max(id,stoi(u.substr(u.find("-")+1))); } else skip=true;
    f.close(); int p=1000+(rand()%9000);
    ofstream of("banker_db.txt",ios::app);
    of<<n<<"\tbanker-"<<id+1<<"\t"<<encrypt(to_string(p))<<"\t50000\n"; of.close();
    cout<<"\nBanker Created:\nName: "<<n<<"\nID: banker-"<<id+1<<"\nPassword: "<<p<<"\n";
}
